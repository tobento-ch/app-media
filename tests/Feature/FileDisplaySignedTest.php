...
use Tobento\App\Media\Feature\FileDisplaySigned;
use Tobento\App\Media\FeaturesInterface;
use Tobento\Service\FileStorage\StoragesInterface;
use Tobento\Service\Routing\RouterInterface;
use function Tobento\App\{directory};

class FileDisplaySignedTest extends \Tobento\App\Testing\TestCase
{
    ...
    public function testFeature()
    {
        $this->fakeConfig()->with('media.features', [
            new FileDisplaySigned(),
        ]);
        
        $app = $this->bootingApp();
        $features = $app->get(FeaturesInterface::class);
        
        $this->assertInstanceof(FileDisplaySigned::class, $features->get('file-display-signed'));
    }
    
    public function testDisplaysFile()
    {
        $this->fakeConfig()->with('media.features', [
            new FileDisplaySigned(supportedStorages: ['private']),
        ]);
        ...
        $url = $router->url('media.file.display.signed', ['storage' => 'private', 'path' => 'path/grünwald.txt'])->sign();
    ...
    public function testReturnsNotFoundResponseIfFileDoesNotExist()
    {
        ...
        $app = $this->bootingApp();
        $router = $app->get(RouterInterface::class);
        $url = $router->url('media.file.display.signed', ['storage' => 'private', 'path' => 'path/not-exists.txt'])->sign();
    ...
    {
        $this->fakeConfig()->with('media.features', [
            new FileDisplaySigned(supportedStorages: ['not-exists']),
        ]);
        ...
        $app = $this->bootingApp();
        $router = $app->get(RouterInterface::class);
        $url = $router->url('media.file.display.signed', ['storage' => 'not-exists', 'path' => 'path/grünwald.txt'])->sign();
    ...
    {
        $this->fakeConfig()->with('media.features', [
            new FileDisplaySigned(supportedStorages: []),
        ]);
    ...
    public function testDisplaysFileUsingCustomRouteUri()
    {
        ...
        $app = $this->bootingApp();
        $router = $app->get(RouterInterface::class);
        $url = (string)$router->url('media.file.display.signed', ['storage' => 'private', 'path' => 'path/grünwald.txt'])->sign();
    ...
    public function testReturnsNotFoundResponseIfStorageIsPublic()
    {
        ...
        $this->fakeConfig()->with('media.features', [
            new FileDisplaySigned(supportedStorages: ['public']),
        ]);
        ...
        $app = $this->bootingApp();
        $router = $app->get(RouterInterface::class);
        $url = (string)$router->url('media.file.display.signed', ['storage' => 'public', 'path' => 'path/file.txt'])->sign();
    ...
    public function testUnsignedUrlFails()
    {
        ...
        $url = (string)$router->url('media.file.display.signed', [
            'storage' => 'private',
            'path' => 'path/file.txt',
        ]);
