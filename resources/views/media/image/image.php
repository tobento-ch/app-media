<div class="image-editor">
    <?php $form = $view->form(); ?>
    <?= $form->form([
        'action' => $view->routeUrl('media.image.editor.update', [
            'template' => $template, 'storage' => $storage, 'path' => $file->path()
        ]),
        'data-image-editor' => [
            'id' => $editorId,
            'previewUrl' => (string)$view->routeUrl('media.image.editor.preview', [
                'template' => $template, 'storage' => $storage, 'path' => $file->path()
            ]),
            'previewMethod' => 'POST',
        ],
            ...
            <div class="image-editor-actions-head">
                <div class="buttons spaced">
                    <?= $form->button(
                        ...
                        attributes: ['class' => 'button primary text-xs', 'data-image-action' => 'save', 'data-image-action-trigger' => 'save']
                    ) ?>
